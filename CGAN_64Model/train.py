...
def compute_gradient_penalty(D, real_samples, fake_samples, one_hot_labels ):
    ...
    d_interpolates = D(interpolates, one_hot_labels)
...
def train_critic(X, Z, D, G, D_solver, G_solver, one_hot_labels):
    d_real = D(X,one_hot_labels)
    fake = G(Z,one_hot_labels)
    d_fake = D(fake,one_hot_labels)
    
    # Gradient penalty
    real_batch = Variable(X.type(Tensor) ) 
    gradient_penalty = compute_gradient_penalty(D, real_batch.data, fake.data, one_hot_labels)
...
def train_gen(Z, D, G, D_solver, G_solver, one_hot_labels):
    fake = G(Z,one_hot_labels)
    d_fake = D(fake,one_hot_labels)
...
def train(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, batch_data in enumerate(dset_loaders):
            X = batch_data[0]
            labels = batch_data[1]
            #2 is the number of classes, might need to change later
            one_hot_labels = torch.FloatTensor(labels.shape[0], 2).zero_()
            one_hot_labels.scatter_(1, labels.unsqueeze(1), 1)
            one_hot_labels = var_or_cuda(one_hot_labels)
            #print(X.shape)
            ...
            #Train the critic
            d_loss ,Wasserstein_D, gp  = train_critic(X ,Z, D, G, D_solver, G_solver, one_hot_labels)
            
            # Train the generator every n_critic steps
            if i % n_critic == 0:
                Z = generateZ(num_samples = X.size(0), z_size = args.z_size)
                g_loss = train_gen(Z, D, G, D_solver, G_solver, one_hot_labels)
        ...
        if (epoch + 1) % args.image_save_step == 0:
            ...
            gen_output = G(Z, one_hot_labels[:8])
