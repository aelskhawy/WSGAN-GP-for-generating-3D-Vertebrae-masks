...
#             torch.nn.ConvTranspose3d(2, self.cube_len*8, kernel_size=4, stride=2, bias=args.bias, padding=(1, 1, 1) ),
#             torch.nn.BatchNorm3d(self.cube_len * 8),
#             torch.nn.ReLU(inplace=True),
#         )
        #IN channels to layer1 is now the z size + 2 (which is number of classes)
        self.layer1 = torch.nn.Sequential(
            torch.nn.ConvTranspose3d(self.args.z_size + 2, self.cube_len*8, kernel_size=4, stride=2, bias=args.bias, padding=padd),
            torch.nn.BatchNorm3d(self.cube_len*8),
            torch.nn.ReLU(inplace = True)
        )
...
class _D(torch.nn.Module):
    def __init__(self, args):
        ...
        if self.cube_len == 32:
            padd = (1,1,1)
        # self.process_labels = torch.nn.Sequential(
        #     torch.nn.Conv3d(2, self.cube_len, kernel_size=4, stride=2, bias=args.bias, padding=(1, 1, 1) ),
        #     torch.nn.LeakyReLU(0.2)
        # )
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv3d(1+2, self.cube_len, kernel_size=4, stride=2, bias=args.bias, padding=(1, 1, 1)),
