...
class VertDataset(Dataset):
    ...
    def __init__(self, root_dir, n=None, transform=None):
        ...
        #Extract vert_num from the file name
        self.vert_nums = [int(file.split(".")[0].split("_")[-1]) for file in self.files_list]
        # assign a label based on the num. from 8 to 19 means "thoracic" label 0
        #from 20 to 24 means lumbar label 1 
        self.labels_list = [0 if 8 <= num <= 19 else 1 for num in self.vert_nums]
    ...
    def __getitem__(self, idx):
        ...
        return sample.double(), label #.unsqueeze(0).float()
